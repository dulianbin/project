...
import com.zhaopin.report.util.CamelCaseUtils;
import com.zhaopin.report.util.annotation.ConvertMapping;
import com.zhaopin.report.util.enums.EnumAware;
...
public class BeanConverter {
    ...
    private static void copySameProperties(Object target, Object source, List<String> ignoreList) {
        ...
        for (PropertyDescriptor targetPd : targetPds) {
            try {
                ...
                System.out.print(targetPd.getName());
    ...
    private static Object customConvert(Class<?> targetPropertyType, Object value) {
        ...
        if (value != null && value.getClass().isEnum()
            && EnumAware.class.isAssignableFrom(value.getClass())) {

            LOG.debug("源对象为枚举类型[EnumAware]");
            result = ((EnumAware) value).getCode();
        }

        if (targetClass.isEnum() && EnumAware.class.isAssignableFrom(targetClass)) {
            LOG.debug("目标对象为枚举类型[EnumAware]");
            targetClass = EnumAware.class;
        }
